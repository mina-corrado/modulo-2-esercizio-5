...
function ricerca(lavoro, geo){
  const ret = [];
  for (let index = 0; index < jobs.length; index++) {
    const element = jobs[index];
    if (element.title.toLowerCase().includes(lavoro.toLowerCase()) && element.location.toLowerCase().includes(geo.toLowerCase())){
      // elimina i campi "description", "requirements", "benefits" e "company_profile"
      delete(element.description);
      delete(element.requirements);
      delete(element.benefits);
      delete(element.company_profile);
      ret.push(element);      
    }
  }
...
let lavoro="dev";
let geo="US";
let risultato=ricerca(lavoro,geo);
console.log(`La ricerca di ${lavoro} e ${geo} ha dato il seguente risultato: `+JSON.stringify(risultato));